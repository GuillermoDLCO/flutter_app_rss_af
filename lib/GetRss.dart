...
class GetRss {
  static Future<List<Noticia>> getRss() async {
    ...
    for (int i = 0; i < rssFeed.items.length; i++) {
      ...
      noticias.add(new Noticia(rssFeed.items[i].title, rssFeed.items[i].description, 
      rssFeed.items[i].pubDate, rssFeed.items[i].enclosure.url, rssFeed.items[i].link));
    }
...
class Noticia {
  ...
  Noticia(this._titulo, this._descripcion, this._fecha, this._urlImagen,
      this._urlNoticia);
